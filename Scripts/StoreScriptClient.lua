...
local TEMP_GROUP = script:GetCustomProperty("TempGroup"):WaitForObject()
...
function OnClick(button)
    if(not inTransaction) then
        ...
        if(thisPlayer:GetResource("Money") >= storeAsset:GetCustomProperty("Price")) then
            --buy the item
            while Events.BroadcastToServer("BuyItem", assetIndex) == BroadcastEventResultCode.EXCEEDED_RATE_LIMIT do
                Task.Wait()
            end
...
function OnBindingPressed(player,bindingName)
    if(bindingName == "ability_extra_28" and (player.clientUserData.UsingCam ~= true or inStore)) then
        if(not inStore) then
...
function OnGameStateChanged(oldState,newState,stateHasDuration,stateEndTime)
    if(newState == ABGS.GAME_STATE_LOAD_MAP) then
